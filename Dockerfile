...
# Install packages from packages.txt with verbose output
RUN cat /var/tmp/packages.txt | xargs -r -d '\n' -I {} sh -c 'apt-get install -yq --no-install-suggests --no-install-recommends {} || echo "Failed to install {}"'
